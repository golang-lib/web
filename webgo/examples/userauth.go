...
import (
	...
	_ "github.com/gopkg/web/webgo/example/sqlite3"
...
var manager = webgo.NewSessionManager(logger)
...
func getSession(ctx *webgo.Context, manager *webgo.SessionManager) *webgo.Session {
	...
	ctx.SetSecureCookie("SessionId", webgo.Id, int64(manager.GetTimeout()))
	ctx.SetHeader("Pragma", "no-cache", true)
	return session
}

func getParam(ctx *webgo.Context, name string) string {
...
func main() {
	...
	manager.OnStart(func(session *webgo.Session) {
		logger.Printf("Start session(\"%s\")", webgo.Id)
	})
	manager.OnEnd(func(session *webgo.Session) {
		logger.Printf("End session(\"%s\")", webgo.Id)
	})
