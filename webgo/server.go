...
func (s *Server) Run(addr string) {
	...
	s.Logger.Printf("webgo.go serving %s\n", addr)
...
func (s *Server) RunFcgi(addr string) {
	s.initServer()
	s.Logger.Printf("webgo.go serving fcgi %s\n", addr)
...
func (s *Server) RunScgi(addr string) {
	s.initServer()
	s.Logger.Printf("webgo.go serving scgi %s\n", addr)
...
// an argument to 'webgo.Ctx' as its first argument
...
// the main route handler in webgo.go
...
func (s *Server) routeHandler(req *http.Request, w http.ResponseWriter) (unused *route) {
	...
	ctx.SetHeader("Server", "webgo", true)
	tm := time.Now().UTC()
